...
public interface PerfumeCategoryRepository extends JpaRepository<PerfumeCategory, Long> {
    ...
    @Query(value = "SELECT p.id FROM peamo.perfumecategory as pc JOIN peamo.api_perfume as p ON pc.perfume_id = p.id WHERE pc.category_id IN (:categoryIds) ORDER BY good_cnt DESC LIMIT 100", nativeQuery = true)
